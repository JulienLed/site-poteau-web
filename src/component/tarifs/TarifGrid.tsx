...
export default function TarifGrid() {
  const { ref: refBase, inView: inViewBase } = useInView({
    triggerOnce: true,
    threshold: 0.15,
  });
  const { ref: refOptions, inView: inViewOptions } = useInView({
    triggerOnce: true,
    threshold: 0.1,
  });
  ...
  return (
    <div className="flex flex-col gap-10">
      {/* Pack de Base — slide depuis la gauche */}
      <div ref={refBase}>
        <motion.div
          initial={{ x: "-100vw", opacity: 0 }}
          animate={inViewBase ? { x: 0, opacity: 1 } : {}}
      ...
      {/* Options — slide depuis la droite + cards en cascade */}
      <div ref={refOptions}>
        <motion.div
          initial={{ x: "100vw", opacity: 0 }}
          animate={inViewOptions ? { x: 0, opacity: 1 } : {}}
          transition={{ duration: 0.7, ease: "easeOut" }}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {supplements.map((s, i) => (
              <motion.div
                ...
                animate={inViewOptions ? { opacity: 1, y: 0 } : { opacity: 0, y: 24 }}
                transition={{ duration: 0.4, delay: i * 0.08, ease: "easeOut" }}
              >
                <Card className="bg-logo-blue/70 border-0 text-sandy-brown shadow-md px-5 py-4 h-full">
